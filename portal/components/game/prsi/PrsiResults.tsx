...
const placementColors = ['text-amber-400', 'text-gray-300', 'text-amber-700', 'text-gray-500'];
const placementBg = ['bg-amber-400/10 border-amber-400/20', 'bg-white/[0.04] border-white/[0.08]', 'bg-amber-700/10 border-amber-700/20', 'bg-white/[0.02] border-white/[0.06]'];
      ...
      <div className="text-center">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-display)]">
          {isWinner ? (
            ...
            <span className="text-gray-300">Konec hry</span>
          )}
        </h2>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium truncate ${isMe ? 'text-white' : 'text-gray-300'}`}>
                    {player.username}
                    {isMe && <span className="text-gray-500 ml-1">(ty)</span>}
                  </p>
                  {player.isGuest && (
                    <p className="text-[10px] text-gray-600">Host</p>
                  )}
                </div>

                {eloChange && eloChange.change !== 0 && (
                  <span className={`text-sm font-mono font-medium ${eloChange.change > 0 ? 'text-green-400' : 'text-red-400'}`}>
