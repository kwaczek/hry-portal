...
export function GameChat({ messages, isGuest, onSend, onReaction, onClose }: GameChatProps) {
  ...
  return (
    <div className="fixed top-14 right-0 bottom-0 z-20 w-full sm:w-80 flex flex-col bg-gray-950/95 backdrop-blur-xl border-l border-white/[0.06] animate-[fadeIn_0.15s_ease-out]">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
        <h3 className="text-sm font-semibold font-[family-name:var(--font-display)] text-gray-200">
          Chat
        </h3>
        <button
          onClick={onClose}
        ...
        <div className="flex gap-1 justify-center">
          {QUICK_REACTIONS.map((emoji) => (
            <button
              ...
              className="w-8 h-8 rounded-md text-base hover:bg-white/[0.08] active:scale-90 transition-all cursor-pointer"
              title={emoji}
            >
              {emoji}
            </button>
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              <input
                ...
                className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-sm text-gray-200 placeholder:text-gray-600 focus:outline-none focus:border-white/[0.15] transition-colors"
              />
            ...
            <button
              type="submit"
              disabled={!text.trim()}
              className="p-2 rounded-lg bg-red-600 text-white disabled:opacity-30 disabled:cursor-not-allowed hover:bg-red-500 transition-colors cursor-pointer"
            >
              <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
  ...
  if (message.type === 'system') {
    return (
      <div className="text-center py-1">
        <span className="text-[11px] text-gray-600 italic">{message.content}</span>
      </div>
    );
  }
